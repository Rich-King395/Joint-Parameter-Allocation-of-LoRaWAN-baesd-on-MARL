...
from CoMAB.CoAgent import CUCB
...
def CoCMAB_SF_train(nodes, episode):
    ...
    CoCMAB_Config.NetworkEnergyEfficiency.append(NetEnergyEfficiency)
    CoCMAB_Config.Network_PDR.append(NetPDR)
    CoCMAB_Config.Network_Throughput.append(NetThroughput)
...
def CoCMAB_TP_allocation(nodes):
    # initialize simulation environment current time for each episode
    env = simpy.Environment()
    ''' initialize the environment at the beginning of episode '''
    reset_simulation_stats()
    ...
    env.run(until=CoCMAB_Config.tp_duration)

    ''' 计算每个节点在上一个发送幕中数据包的平均RSSI '''
    average_rssi = [sum(rssi_list) / len(rssi_list) if rssi_list else None for rssi_list in CoCMAB_Config.node_rssi_list]
    ...
    CoCMAB_Config.tp_allocation_flag = 1

# '''训练结束后, 测试训练效果'''
def CoMAB_eval(nodes):
     # initialize simulation environment current time for each episode
    env = simpy.Environment()
    ...
    env.run(until=CoCMAB_Config.eval_duration)
    ...
    print("Evaluation结束")
    print(f"Evaluation得到的网络指标为: PDR={NetPDR*100:.2f}, Network EE={NetEnergyEfficiency:.2f}, Network Throughput={NetThroughput:.2f},")
    ...
    while True:
        ...
        for bs in range(0, nrBS):
            if node in ParameterConfig.packetsAtBS[bs]:
                ...
                if CoCMAB_Config.sf_train_flag == 1 and CoCMAB_Config.tp_allocation_flag == 0:
                    CoCMAB_Config.node_rssi_list[node.id].append(node.packet[bs].RSSI)
