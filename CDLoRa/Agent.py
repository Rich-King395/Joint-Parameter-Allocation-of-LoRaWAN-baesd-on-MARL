...
class CUCB(CDLoRa):
    ...
    def actions_choose(self):
        ...
        '''S choose'''
        ucb_sf = self.Q_SF + CDLoRa_Config.maximum_sf_reward * np.sqrt(
            (np.log(self.total_count)) / (2 * (self.counts_SF + 1)))  # calculate ucb of sf
        k_sf = np.argmax(ucb_sf)
        self.counts_SF[k_sf] += 1
        sf_index = self.sf_arms[k_sf]

        '''Transmission power choose'''
        ucb_tp = self.Q_TP + CDLoRa_Config.maximum_tp_reward * np.sqrt(
