...
def NaiveMAB_run(nodes):
    # set_seed(random_seed)
    for episode in range(NaiveMAB_Config.num_episode):
        ParameterConfig.global_episode = episode
        NaiveMAB_train(nodes, episode)

    training_process(NaiveMAB_Config)

    NaiveMAB_eval(nodes)

    result_record(NaiveMAB_Config.NetPDR, NaiveMAB_Config.NetEnergyEfficiency)              

''' 每一幕的数据包参数配置和所有智能体基础臂的期望奖励更新 '''
def NaiveMAB_train(nodes, episode):
    ...
    for node in nodes:
        ...
        env.process(NaiveMAB_transmit(env,node))
        # env.process(transmit(env,node))

    # active_processes = len(env._queue)
    # print(f"当前环境中运行的进程数: {active_processes}")

    env.run(until=NaiveMAB_Config.eposide_duration)
    ...
    NaiveMAB_Config.NetworkEnergyEfficiency.append(NetEnergyEfficiency)
    NaiveMAB_Config.Network_PDR.append(NetPDR)
    NaiveMAB_Config.Network_Throughput.append(NetThroughput)
...
def NaiveMAB_eval(nodes):
    ...
    env.run(until=NaiveMAB_Config.eval_duration)
    ...
    NaiveMAB_Config.NetPDR = float(len(ParameterConfig.recPackets)/len(ParameterConfig.sentPackets)) 
    NaiveMAB_Config.NetThroughput = 8 * float(ParameterConfig.RecPacketSize) / ParameterConfig.TotalPacketAirtime
    NaiveMAB_Config.NetEnergyEfficiency = float(8*ParameterConfig.RecPacketSize / ParameterConfig.TotalEnergyConsumption)

    sf_distribution(NaiveMAB_Config.result_folder_path)
    tp_distribution(NaiveMAB_Config.result_folder_path)

    print("Evaluation结束")
    print(f"Evaluation得到的网络指标为: PDR={NaiveMAB_Config.NetPDR*100:.2f}, Network EE={NaiveMAB_Config.NetEnergyEfficiency:.2f}, Network Throughput={NaiveMAB_Config.NetThroughput:.2f},")

def NaiveMAB_transmit(env,node):
    while True:
        # set_seed(random_seed)
        ...
        node.agent.Expected_Reward_Update(node.arm_index)
...
def record(episode, PDR, EE):
    base_result_folder_path = f"/home/uestc/LoRaSimulator/Joint-Parameter-Allocation-of-LoRaWAN-baesd-on-MARL/NaiveMAB/results"
    ...
    NaiveMAB_Config.result_folder_path = os.path.join(base_result_folder_path, subfolder_name)

    if not os.path.exists(NaiveMAB_Config.result_folder_path):
        os.makedirs(NaiveMAB_Config.result_folder_path)
    
    record_file = "record.txt"
    record_file_path = os.path.join(NaiveMAB_Config.result_folder_path, record_file)
...
def result_record(PDR, EE):
    ...
    result_file_path = os.path.join(NaiveMAB_Config.result_folder_path, result_file)
    with open(result_file_path, 'w') as file:
        ...
        file.write('PDR={:.3f} %,'.format(NaiveMAB_Config.Network_PDR[-1]*100))
        file.write('Network EE={:.3f} bits/mJ\n'.format(NaiveMAB_Config.NetworkEnergyEfficiency[-1]))
