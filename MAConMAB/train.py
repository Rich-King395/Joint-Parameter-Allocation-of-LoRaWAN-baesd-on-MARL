...
import random
def MAConMAB_run(nodes):
    # set_seed(random_seed)

    for episode in range(MAConMAB_Config.sf_num_episode):
        MAConMAB_train(nodes, episode)

    plot()
    
    MAConMAB_Config.sf_train_flag = 1

    MAConMAB_TP_allocation(nodes)
        
    MAConMAB_eval(nodes)

    result_record(MAConMAB_Config.NetPDR, MAConMAB_Config.NetEnergyEfficiency)

''' 每一幕的数据包参数配置和所有智能体基础臂的期望奖励更新 '''
def MAConMAB_train(nodes, episode):
    ...
    for node in nodes: 
        ...
        # node.fre_index = 0
        # node.fre_index = random.randint(0,7)
        # node.sf_index = random.randint(0,5)

        if episode % 3 == 0:
            node.sf_index = random.randint(0, 5)
        else:
            max_val = np.sum(MAConMAB_Config.contexts[node.fre_index])
            context = MAConMAB_Config.contexts[node.fre_index] / max_val
            # print("context:", context)
            node.sf_index = node.agent.actions_choose(context)
        ...
        env.process(MAConMAB_transmit(env,node))
        #env.process(transmit(env,node))

    # active_processes = len(env._queue)
    # print(f"当前环境中运行的进程数: {active_processes}")

    env.run(until=MAConMAB_Config.eposide_duration)

    
    MAConMAB_Config.contexts = [np.array([0, 0, 0, 0, 0, 0]) for _ in range(numChannel)] 

    for node in nodes:
        MAConMAB_Config.contexts[node.fre_index][node.sf_index] += 1
    ...
    for node in nodes:
        ...
        node.agent.update_linucb(context, reward, node.sf_index)

        # if node.id == 1:
        #     print("MAConMAB_Config.contexts: ",MAConMAB_Config.contexts) 
    ...
    #     sumSec={sumRec},ParameterConfig.recPackets={len(ParameterConfig.recPackets)},ParameterConfig.recPackets={len(ParameterConfig.recPackets)}")


    if episode % 3:
        MAConMAB_Config.NetworkEnergyEfficiency.append(NetEnergyEfficiency)
        MAConMAB_Config.Network_PDR.append(NetPDR)
        MAConMAB_Config.Network_Throughput.append(NetThroughput)

    if episode % 3:
        print(f"episode={episode}, PDR={NetPDR*100:.2f}, Network EE={NetEnergyEfficiency:.2f}, Network Throughput={NetThroughput:.2f},")

def MAConMAB_TP_allocation(nodes):
    # initialize simulation environment current time for each episode
    env = simpy.Environment()
    ''' initialize the environment at the beginning of episode '''
    reset_simulation_stats()
    ...
    set_seed(random_seed)
    env.run(until=MAConMAB_Config.tp_duration)

    ''' 计算每个节点在上一个发送幕中数据包的平均RSSI '''
    average_rssi = [sum(rssi_list) / len(rssi_list) if rssi_list else None for rssi_list in MAConMAB_Config.node_rssi_list]
    ...
    for node in nodes:
        ...
        if delta_Power > 0:
            ...
            if new_tp < 8:
                node.tp_index = np.where(Transmission_Power == new_tp)[0] + 4
            else:
                node.tp_index = np.where(Transmission_Power == new_tp)[0]

'''训练结束后, 测试训练效果'''
def MAConMAB_eval(nodes):
    # initialize simulation environment current time for each episode
    env = simpy.Environment()
    ...
    env.run(until=MAConMAB_Config.eval_duration)
        
    MAConMAB_Config.NetPDR = float(len(ParameterConfig.recPackets)/len(ParameterConfig.sentPackets)) 
    MAConMAB_Config.NetThroughput = 8 * float(ParameterConfig.RecPacketSize) / ParameterConfig.TotalPacketAirtime
    MAConMAB_Config.NetEnergyEfficiency = float(8*ParameterConfig.RecPacketSize / ParameterConfig.TotalEnergyConsumption)

    print("Evaluation结束")
    print(f"Evaluation得到的网络指标为: PDR={MAConMAB_Config.NetPDR*100:.2f}, Network EE={MAConMAB_Config.NetEnergyEfficiency:.2f}, Network Throughput={MAConMAB_Config.NetThroughput:.2f},")

def MAConMAB_transmit(env,node):
    while True:
        # set_seed(random_seed)
        ...
        # print("node.PacketPara.sf:",node.PacketPara.sf,"node.PacketPara.tp:",node.PacketPara.tp,"node.PacketPara.fre:",node.PacketPara.fre) 
        ...
        for bs in range(0, nrBS):
            if node in packetsAtBS[bs]:
                ...
                if MAConMAB_Config.sf_train_flag == 1:
                    MAConMAB_Config.node_rssi_list[node.id].append(node.packet[bs].RSSI)
...
def CMAB_Generate_Packet(node):
    ...
    for i in range(0,nrBS):
        ...
        node.PacketPara.tp = Transmission_Power[node.tp_index]  
        # print("node.PacketPara.sf:",node.PacketPara.sf,"node.PacketPara.tp",node.PacketPara.tp)
...
def plot():
    base_result_folder_path = f"/home/uestc/LoRaSimulator/Joint-Parameter-Allocation-of-LoRaWAN-baesd-on-MARL/MAConMAB/results"
    ...
    plt.figure()
    x = [i for i in range(1, MAConMAB_Config.sf_num_episode + 1) if i % 3 != 0]
...
def result_record(PDR, EE):
    ...
    result_file_path = os.path.join(MAConMAB_Config.result_folder_path, result_file)
    with open(result_file_path, 'w') as file:
        ...
        file.write('PDR={:.3f} %,'.format(MAConMAB_Config.Network_PDR[-1]*100))
        file.write('Network EE={:.3f} bits/mJ\n'.format(MAConMAB_Config.NetworkEnergyEfficiency[-1]))
