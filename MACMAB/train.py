...
def MACMAB_run(nodes):
    ...
    plot("/home/uestc/LoRaSimulator/Joint-Parameter-Allocation-of-LoRaWAN-baesd-on-MARL/MACMAB/results")
...
def MACMAB_train(nodes, episode):
    ...
    MACMAB_Config.channel_sf_count = [[0 for _ in range(numSF)] for _ in range(numChannel)]
    ...
    for node in nodes:
        ...
        node.sf_bw_index, node.tp_index = node.agent.actions_choose()

        # print("节点", node.id, "在第", episode, "幕选择的SF+BW为:", node.agent.sf_bw_arms[node.sf_bw_index],"选择的TP为:", node.agent.tp_arms[node.tp_index])
        MACMAB_Config.channel_sf_count[node.fre_index][node.sf_index] += 1
    ...
    for node in nodes:
        ...
        '''SF+BW基础臂的奖励'''
        reward_towords_sf_BW = float((Bandwidth[node.bw_index]/(2^(SF[node.sf_index])))/(500/(2^7)))
        
        ###奖励使SF分配趋向公平###
        ideal = len(MACMAB_Config.channel_sf_count[node.fre_index]) / numSF
        reward_towards_fair_sf = 1 / (1 + MACMAB_Config.channel_sf_count[node.fre_index][node.sf_index] / ideal)

        node.agent.reward_SF_BW = node.PDR + reward_towords_sf_BW + reward_towards_fair_sf

        # print("节点", node.id, "在第", episode, "幕SF+BW奖励为",node.agent.reward_SF_BW)
        '''TP基础臂的奖励'''
        reward_towards_EE = 1 - float(Transmission_Power[node.tp_index]/TP_SUM)
        node.agent.reward_TP = node.PDR + reward_towards_EE

        '''对两个基础臂期望奖励进行更新'''
        node.agent.Expected_Reward_Update(node.sf_bw_index, node.tp_index)

    print(f"episode={episode}, PDR={NetPDR*100:.2f}, Network Throughput={NetThroughput:.2f}, Network EE={NetEnergyEfficiency:.2f}")
...
def CMAB_Generate_Packet(node):
    ...
    for i in range(0,nrBS):
        ...
        PacketPara.sf = SF[node.agent.sf_bw_arms[node.sf_bw_index][0]]
        PacketPara.bw = Bandwidth[node.agent.sf_bw_arms[node.sf_bw_index][1]]
