...
def ILCMAB_run(nodes):
    ...
    for episode in range(ILCMAB_Config.num_episode):
    ...
    result_record(ILCMAB_Config.NetPDR, ILCMAB_Config.NetEnergyEfficiency)              
...
def ILCMAB_train(nodes, episode):
    ...
    env.run(until=ILCMAB_Config.eposide_duration)
    ...
    ILCMAB_Config.NetworkEnergyEfficiency.append(NetEnergyEfficiency)
    ILCMAB_Config.Network_PDR.append(NetPDR)
    ILCMAB_Config.Network_Throughput.append(NetThroughput)

    print(f"episode={episode}, PDR={NetPDR*100:.2f}, Network EE={NetEnergyEfficiency:.2f}, Network Throughput={NetThroughput:.2f},")


'''训练结束后, 测试训练效果'''
def ILCMAB_eval(nodes):
    # initialize simulation environment current time for each episode
    env = simpy.Environment()
    ...
    env.run(until=ILCMAB_Config.eval_duration)
    ...
    ILCMAB_Config.NetPDR = float(len(ParameterConfig.recPackets)/len(ParameterConfig.sentPackets)) 
    ILCMAB_Config.NetThroughput = 8 * float(ParameterConfig.RecPacketSize) / ParameterConfig.TotalPacketAirtime
    ILCMAB_Config.NetEnergyEfficiency = float(8*ParameterConfig.RecPacketSize / ParameterConfig.TotalEnergyConsumption)

    print("Evaluation结束")
    print(f"Evaluation得到的网络指标为: PDR={ILCMAB_Config.NetPDR*100:.2f}, Network EE={ILCMAB_Config.NetEnergyEfficiency:.2f}, Network Throughput={ILCMAB_Config.NetThroughput:.2f},")
    ...
    while True:
        ...
        for bs in range(0, nrBS):
            if node.packet[bs].lost == True or node.packet[bs].collided == 1: 
                ...
                reward_towards_EE = float(Transmission_Power[node.tp_index] / TP_SUM)
                # reward_towards_EE = 0
                node.agent.reward_TP = 1 - 1.8 * reward_towards_EE # TP基础臂的奖励
...
def plot():
    base_result_folder_path = f"/home/uestc/LoRaSimulator/Joint-Parameter-Allocation-of-LoRaWAN-baesd-on-MARL/ILCMAB/results"
    subfolder_name = f"{nrNodes}_nodes_{radius}_m"
    ILCMAB_Config.result_folder_path = os.path.join(base_result_folder_path, subfolder_name)

    # 如果文件夹不存在则创建
    if not os.path.exists(ILCMAB_Config.result_folder_path):
        os.makedirs(ILCMAB_Config.result_folder_path)

    '''Network Energy Efficiency'''
    plt.figure()
    x = range(1,ILCMAB_Config.num_episode+1)
    plt.plot(x, ILCMAB_Config.NetworkEnergyEfficiency, label='NetworkEnergyEfficiency', color='blue')
    ...
    plt.savefig(os.path.join(ILCMAB_Config.result_folder_path, fig1_name))

    '''Network PDR'''
    plt.figure()
    plt.plot(x, ILCMAB_Config.Network_PDR, '-', color='b')
    plt.title('Network PDR changes with increasing episode')
    plt.xlabel('Episode')
    plt.ylabel('Network PDR')

    fig2_name = 'Network_PDR_plot.png'
    plt.savefig(os.path.join(ILCMAB_Config.result_folder_path, fig2_name))

    '''Network Throughput'''
    plt.figure()
    plt.plot(x, ILCMAB_Config.Network_Throughput, '-', color='b')
    ...
    plt.savefig(os.path.join(ILCMAB_Config.result_folder_path, fig3_name))

def result_record(PDR, EE):
    result_file = "Result.txt"
    result_file_path = os.path.join(ILCMAB_Config.result_folder_path, result_file)
    with open(result_file_path, 'w') as file:
        ...
        file.write('PDR={:.3f} %,'.format(ILCMAB_Config.Network_PDR[-1]*100))
        file.write('Network EE={:.3f} bits/mJ\n'.format(ILCMAB_Config.NetworkEnergyEfficiency[-1]))
