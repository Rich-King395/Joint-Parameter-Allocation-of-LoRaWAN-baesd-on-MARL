...
from MARL.utils import setup_seed
import random
def MAB_train(nodes):
    # episode_reward_lst = []
    # log = defaultdict(list)
    file_name = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    folder_path = os.path.join(os.getcwd(), "MAB-results")
    folder_path = os.path.join(folder_path,file_name)
    if not os.path.exists(folder_path):
            os.makedirs(folder_path)
    result_file_name = "MAB-traning-result.txt"
    file_path = os.path.join(folder_path, result_file_name)

    setup_seed(MAB_Config.random_seed)
