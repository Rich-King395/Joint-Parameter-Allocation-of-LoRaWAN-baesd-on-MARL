...
def CDLoRa_run(nodes):
    # set_seed(random_seed)

    for episode in range(CDLoRa_Config.num_episode):
        ParameterConfig.global_episode = episode
        CDLoRa_train(nodes, episode)

    training_process(CDLoRa_Config)

    CDLoRa_eval(nodes)

    result_record(CDLoRa_Config.NetPDR, CDLoRa_Config.NetEnergyEfficiency)              

''' 每一幕的数据包参数配置和所有智能体基础臂的期望奖励更新 '''
def CDLoRa_train(nodes, episode):
    ...
    for node in nodes:
        ...
        env.process(CDLoRa_transmit(env,node))
        # env.process(transmit(env,node))

    # active_processes = len(env._queue)
    # print(f"当前环境中运行的进程数: {active_processes}")

    env.run(until=CDLoRa_Config.eposide_duration)
    ...
    CDLoRa_Config.NetworkEnergyEfficiency.append(NetEnergyEfficiency)
    CDLoRa_Config.Network_PDR.append(NetPDR)
    CDLoRa_Config.Network_Throughput.append(NetThroughput)
...
def CDLoRa_eval(nodes):
    ...
    env.run(until=CDLoRa_Config.eval_duration)
    ...
    CDLoRa_Config.NetPDR = float(len(ParameterConfig.recPackets)/len(ParameterConfig.sentPackets)) 
    CDLoRa_Config.NetThroughput = 8 * float(ParameterConfig.RecPacketSize) / ParameterConfig.TotalPacketAirtime
    CDLoRa_Config.NetEnergyEfficiency = float(8*ParameterConfig.RecPacketSize / ParameterConfig.TotalEnergyConsumption)

    sf_distribution(CDLoRa_Config.result_folder_path)
    tp_distribution(CDLoRa_Config.result_folder_path)

    print("Evaluation结束")
    print(f"Evaluation得到的网络指标为: PDR={CDLoRa_Config.NetPDR*100:.2f}, Network EE={CDLoRa_Config.NetEnergyEfficiency:.2f}, Network Throughput={CDLoRa_Config.NetThroughput:.2f},")



def CDLoRa_transmit(env,node):
    while True:
        # set_seed(random_seed)
...
def record(episode, PDR, EE):
    base_result_folder_path = f"/home/uestc/LoRaSimulator/Joint-Parameter-Allocation-of-LoRaWAN-baesd-on-MARL/CDLoRa/results"
    if CAASI_flag == 1:
    ...
    CDLoRa_Config.result_folder_path = os.path.join(base_result_folder_path, subfolder_name)

    if not os.path.exists(CDLoRa_Config.result_folder_path):
        os.makedirs(CDLoRa_Config.result_folder_path)
    
    record_file = "record.txt"
    record_file_path = os.path.join(CDLoRa_Config.result_folder_path, record_file)
...
def result_record(PDR, EE):
    ...
    result_file_path = os.path.join(CDLoRa_Config.result_folder_path, result_file)
    with open(result_file_path, 'w') as file:
        ...
        file.write('PDR={:.3f} %,'.format(CDLoRa_Config.Network_PDR[-1]*100))
        file.write('Network EE={:.3f} bits/mJ\n'.format(CDLoRa_Config.NetworkEnergyEfficiency[-1]))
